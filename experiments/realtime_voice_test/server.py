...
def create_app() -> FastAPI:
    ...
    async def ws_endpoint(websocket: WebSocket, call_sid: str) -> None:
        ...
        # Pipeline-tap audio recorder. Caller (pipeline rate) and bot
        # (OpenAI's 24 kHz) audio arrive at different sample rates; the
        # tap resamples both to PIPELINE_SAMPLE_RATE on the fly so the
        # output is a single WAV at one rate that plays both directions
        # at correct speed.
        recording_tap = RecordingTapProcessor(target_sample_rate=PIPELINE_SAMPLE_RATE)
