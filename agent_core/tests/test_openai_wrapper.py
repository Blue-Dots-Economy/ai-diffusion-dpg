"""
agent_core/tests/test_openai_wrapper.py

Unit tests for OpenAILLMWrapper.
...
Coverage:
  ...
  - llm_base_url is forwarded to the OpenAI client
...
@patch("src.llm_wrapper.openai_wrapper.openai.OpenAI")
@patch("src.llm_wrapper.openai_wrapper.openai.AsyncOpenAI")
def test_init_forwards_base_url_to_client(mock_async_cls, mock_cls):
    config = {**VALID_CONFIG, "llm_base_url": "https://my-endpoint.com/v1"}
    OpenAILLMWrapper(config)
...
def test_call_returns_text_response(mock_cls, mock_async_cls):
    ...
    wrapper = OpenAILLMWrapper(VALID_CONFIG)
...
@patch("src.llm_wrapper.openai_wrapper.time.sleep")
@patch("src.llm_wrapper.openai_wrapper.openai.AsyncOpenAI")
@patch("src.llm_wrapper.openai_wrapper.openai.OpenAI")
...
@patch("src.llm_wrapper.openai_wrapper.asyncio.sleep", new_callable=AsyncMock)
@patch("src.llm_wrapper.openai_wrapper.openai.AsyncOpenAI")
@patch("src.llm_wrapper.openai_wrapper.openai.OpenAI")
