...
# ---------------------------------------------------------------------------
# GET /profile/{user_id} — GH-151 follow-up stub endpoint
# ---------------------------------------------------------------------------


def test_get_profile_returns_profile_dict(client, mock_memory):
    """Populated profile flows through _user_store.get_profile."""
    mock_memory._user_store.get_profile.return_value = {
        "trade": "electrician",
        "location": "Hubli",
        "attributes": [],
    }
    response = client.get("/profile/+919876543210")
    assert response.status_code == 200
    assert response.json() == {
        "trade": "electrician",
        "location": "Hubli",
    ...
    mock_memory._user_store.get_profile.assert_called_once_with("+919876543210")


def test_get_profile_unknown_user_returns_empty_dict(client, mock_memory):
    """No profile found → {} (never a 404)."""
    mock_memory._user_store.get_profile.return_value = {}
...
def test_get_profile_memory_exception_returns_empty_dict(client, mock_memory):
    """Memgraph errors must never propagate to the tool caller."""
    mock_memory._user_store.get_profile.side_effect = RuntimeError("memgraph down")
    response = client.get("/profile/+919876543210")
    assert response.status_code == 200
    assert response.json() == {}


def test_get_profile_strips_whitespace(client, mock_memory):
    """Keep the endpoint forgiving about stray whitespace in the user_id."""
    mock_memory._user_store.get_profile.return_value = {"trade": "fitter"}
    response = client.get("/profile/%20%20user%20%20")
    assert response.status_code == 200
    assert response.json() == {"trade": "fitter"}
    mock_memory._user_store.get_profile.assert_called_once_with("user")
