...
def get_phase_addition(phase: str, available_tools: list[str] | None = None) -> str:
    ...
    if phase == "tools":
        return (
            ...
            "⚠️ **CRITICAL — DO NOT rename connector params after add_rest_api_tool.**\n"
            "The auto-generated connector's `input_schema.properties` keys must MATCH the\n"
            "tool's `source: agent` param names exactly, because the REST adapter forwards\n"
            "the LLM's params verbatim into the HTTP request. If you call\n"
            "`update_config(block=agent_core, section=connectors.read, ...)` to rename a\n"
            "tool's agent-sourced param (call it `<orig>`) into a different connector key\n"
            "(`<renamed>`), the LLM will send `?<renamed>=...` to an API expecting\n"
            "`?<orig>=...` and the call will silently fail with empty results or a 400.\n"
            "- ✅ DO: edit the connector's `description`, `invocation_rules`, or any\n"
            "  per-property `description` text — these are LLM-facing hints only.\n"
            "- ❌ DO NOT: rename property keys inside `input_schema.properties`.\n"
            "- If you genuinely want a different name visible to the LLM, change the\n"
            "  tool's `source: agent` param name itself (the API receives whatever the\n"
            "  tool param is named, so they must be edited together).\n"
    ...
    if phase == "workflow":
        ...
        return (
            ...
            "Before you create or update a single subagent, look at "
            "`agent_core.preprocessing.nlu_processor.intents` in the config state above "
            "and write down what's there. Those names are the SOURCE OF TRUTH.\n\n"
