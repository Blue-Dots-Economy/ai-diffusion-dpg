...
def get_phase_addition(phase: str, available_tools: list[str] | None = None) -> str:
    ...
    if phase == "overview":
        return (
            ...
            "**Do NOT ask about any of these — they are not needed:**\n"
    ...
    if phase == "language":
        return (
            ...
            "English loanwords) under `channels.voice.tts_rules`\n"
            "- **Voice only (skip if voice not in selected_channels):** "
            "`channels.voice.terminal_word` — the literal word that signals call end "
            "(e.g. \"Goodbye\"). Required for voice.\n\n"
            ...
            "+ terminal word: `section=channels, "
            ...
            "(TTS rules, terminal_word, channels.voice). Do not ask about them.\n\n"
            ...
            "**Default recommendation:** Use `claude-haiku-4-5-20251001` as primary for "
            "cost-effective agents (100-500 users/day). Use `claude-sonnet-4-6` as fallback. "
            "For complex reasoning agents, use `claude-sonnet-4-6` as primary with "
            "`claude-haiku-4-5-20251001` as fallback.\n"
            ...
            "**Group 3 (voice only) — TTS rules & terminal word:**\n"
            "Only present this group if voice is in selected_channels. Show TTS rules "
            "and terminal word with suggested defaults. Skip entirely for non-voice agents.\n\n"
            ...
            "selected_channels) channels.voice.{tts_rules, terminal_word} are all set, "
            "call `set_phase('knowledge')`."
        )
    ...
    if phase == "knowledge":
        return (
            ...
            "### Conversation style for this phase\n"
            "Present the knowledge configuration as **one block** with suggested defaults:\n"
            "- collection_name (derived from project slug)\n"
            "- persona text (based on the agent's role and target users)\n"
            "- language_instruction (based on the agent's languages)\n"
            "- RAG settings (top_k, similarity_threshold, vector_store)\n"
            "Show all together and ask: 'Here is the suggested knowledge configuration — "
            "do these look good, or would you like to change any?'\n"
    ...
    if phase == "reach":
        return (
            ...
            "Suggest TTS provider, telephony adapter, and any voice-specific settings "
            "in one block.\n\n"
            "```yaml\n"
