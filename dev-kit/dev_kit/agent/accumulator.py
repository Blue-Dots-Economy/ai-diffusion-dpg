...
class ConfigAccumulator:
    ...
    def get_required_channel_secrets(self) -> list[dict]:
        ...
        if "voice" in selected:
            # When recording is opted in, the caller_id_hash_salt and any S3
            # KMS key are treated as secrets (entered in Deployment Inputs, not
            # stored as plaintext in YAML). We surface them here so the deploy
            # wizard renders the appropriate masked credential fields.
            recording_cfg = (
